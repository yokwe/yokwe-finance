...
public class UpdateStockCodeName extends UpdateBase {
	...
	public void update() {
		...
		for(var file: jsonFileList) {
			...
			if (stockDetail.section1 == null || stockDetail.section1.data == null) {
				continue;
			}
			for(var e: stockDetail.section1.data.values()) {
				// skip Tokyo Pro Markets
				if (e.LISS_CNV.equals("TPM")) {
					continue;
				}
				// skip if no history price  -- before inception date
				if (e.A_HISTDAYL.isEmpty()) {
					continue;
				}

				var stockCode = StockCodeJP.toStockCode5(e.TTCODE.replace("/T", ""));
				var isinCode  = e.ISIN;
				var name      = StringUtil.toFullWidth(e.FLLN).replace("　　", "　");

				var type = typeMap.get(stockCode);
				if (type == null) {
					type = typeMap.get(e.LISS_CNV);
				}
				if (type == null) {
