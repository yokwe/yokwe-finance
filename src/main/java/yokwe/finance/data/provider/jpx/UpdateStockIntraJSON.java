...
public class UpdateStockIntraJSON extends UpdateComplexTask<CodeName> {
	private static final org.slf4j.Logger logger = yokwe.util.LoggerUtil.getLogger();

	public static Makefile MAKEFILE = Makefile.builder().
		input(StorageJPX.StockListJPX).
		output(StorageJPX.StockIntraJSON).
		build();
	...
	public static List<File> getJSONFileList() {
		var array = StorageJPX.StockIntraJSON.getDir().listFiles(o -> o.isFile() && o.getName().endsWith(".json"));
	...
	protected void delistUnknownFile(List<CodeName> stockList) {
		...
		StorageJPX.StockIntraJSON.delistUnknownFile(validNameList);
	}
	...
	protected List<Task> getTaskList(List<CodeName> stockList) {
		...
		for(var stock: stockList) {
			var code = stock.code;
			var file = StorageJPX.StockIntraJSON.getFile(code);
	...
	protected void updateFile(List<CodeName> stockList) {
		// touch file
		StorageJPX.StockIntraJSON.touch();
	}
}
